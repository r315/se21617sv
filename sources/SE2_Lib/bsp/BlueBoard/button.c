...
uint32_t BUTTON_Capture(void){
    return (~(BUTTON_PORT)) & BUTTON_MASK;  //common ground
}

void BUTTON_Init(int holdtime){
    __button.cur  = BUTTON_EMPTY;
    __button.last = BUTTON_EMPTY;
    __button.events = BUTTON_EMPTY;
    __button.htime = holdtime;
    ...
    switch(__button.events){
        ...
        case BUTTON_PRESSED:
            ...
            if(cur == __button.cur){             // same key still pressed
                ...
                __button.counter = TIME_GetValue();
                break;
            }
        ...
        case BUTTON_TIMING:
            ...
            if(cur == __button.cur){
                if(Time_Elapsed(__button.counter) > __button.htime){
